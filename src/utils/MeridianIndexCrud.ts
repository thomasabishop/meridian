...
export class MeridianIndexCrud {
   ...
   > {
      return await this.workspaceContextUtils.readFromWorkspaceContext("MERIDIAN")
   }

   private async writeWorkspaceIndex(index: Record<string, IMeridianEntry>) {
      await this.workspaceContextUtils.writeToWorkspaceContext("MERIDIAN", index)
   }

   public async getMeridianEntry(key: string): Promise<IMeridianEntry | undefined> {
   ...
   public async updateMeridianEntry(key: string, payload: IMeridianEntry): Promise<void> {
      const workspaceIndex = await this.readWorkspaceIndex()
